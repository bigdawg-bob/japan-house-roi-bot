...
print("V8.4 start - using RSS feed")
requests.post(f"https://api.telegram.org/bot{BOT_TOKEN}/sendMessage", json={"chat_id":CHAT_ID,"text":"V8.4 started - pulling live listings from RSS feed..."})

# GET LINKS FROM RSS FEED - reliable
links=[]
try:
    ...
    print(f"RSS fail {e}, fallback to homepage scrape")
    try:
...
print(f"Final links to check: {len(links)}")
...
for lk in links:
    try:
        ...
        data=deepseek_safe(f"URL {lk} price {man}man ({usd}K USD) snippet {pg[:3500]} Rules: Hakuba=10, Atami=9.5, Kyoto max 6, popularity>yield, LIVE check passed")
        if not data:
            print(f"Deepseek None, fallback rating 7")
            data={"hook":f"${usd}K JAPAN HOUSE","sub_hook":"5 MINS TO JR STATION","caption":f"💰 ${usd}K USD | Live verified\n**Net Rental Income: $8K / year (10% yield)**\n📜 LICENSE: New Minpaku 180 days\n⭐ RATING: 7/10\nSource: {lk}","rating":7}
...
if not posted:
    requests.post(f"https://api.telegram.org/bot{BOT_TOKEN}/sendMessage", json={"chat_id":CHAT_ID,"text":f"Finished checked {len(links)} LIVE links, none posted. All <6 or filtered as SOLD."})
print("Done V8.4")
